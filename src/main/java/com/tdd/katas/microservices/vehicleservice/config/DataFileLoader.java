...
public class DataFileLoader {
    ...
    private boolean processFile(File dataFile) {
        Map<String,VehicleData> vehicleDataMap;
        try {
            vehicleDataMap = objectMapper.readValue(dataFile, new TypeReference<Map<String,VehicleData>>(){});
            logger.info("Found: {} entries", vehicleDataMap.size());
        } catch (IOException e) {
        ...
        for (Map.Entry<String,VehicleData> vehicleData : vehicleDataMap.entrySet()) {
            vehicleRepository.createVehicleData(vehicleData.getKey(), vehicleData.getValue());

            try {
                logger.info("Created entry: [{}]", objectMapper.writeValueAsString(vehicleData));
            }
            catch (Exception e) {
                logger.info("Created entry: [{}]", vehicleData.toString());
            }
        }
        return true;
    }
